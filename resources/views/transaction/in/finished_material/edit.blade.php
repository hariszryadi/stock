...
<main id="main-container">
    ...
    <div class="content">
        <div class="block block-rounded">
            <form action="{{ route('finished_material_in.update', $tfm->id) }}" method="POST">
                @csrf
                @method('PUT')
                        ...
                        <div class="mb-4">
                            @foreach ($tfm->detail as $key => $i)
                                <div class="row row-block {{ $key != 0 ? 'mt-4' : '' }}">
                                    ...
                                    <input type="hidden" name="code[]" value="{{ $i->finished_material_id }}">
                                    <div class="col-sm-9">
                                        ...
                                        <select name="code[]" id="code.{{ $key }}" class="form-control js-select2" disabled>
                                            <option value="" selected disabled>-- Pilih --</option>
                                            @foreach ($fm as $item)
                                                <option value="{{ $item->code }}" {{ $i->finished_material_id == $item->id ? 'selected' : '' }}>{{ $item->code }} - {{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('code.{{ $key }}')
