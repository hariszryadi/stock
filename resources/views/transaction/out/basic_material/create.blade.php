...
<main id="main-container">
            ...
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
                ...
                <nav class="flex-shrink-0 mt-3 mt-sm-0 ms-sm-3" aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-alt">
                        ...
                        <li class="breadcrumb-item">
                            <a class="link-fx" href="{{ route('basic_material_out.index') }}">{{ $title }}</a>
                        </li>
    ...
    <div class="content">
        ...
        <div class="block block-rounded">
            <form action="{{ route('basic_material_out.store') }}" method="POST">
                @csrf
                            ...
                            <div class="row row-block">
                                <div class="col-sm-9">
                                    ...
                                    <select name="code[]" id="code.0" class="form-control js-select2 @error('code.0') is-invalid @enderror">
                                        ...
                                        @foreach ($bm as $item)
                                            <option value="{{ $item->id }}" {{ old('code.0') == $item->id ? 'selected' : '' }}>{{ $item->code }} - {{ $item->name }} - ({{ $item->qty }})</option>
                                        @endforeach
                                    </select>
                                    @error('code.0')
                                        <div class="invalid-feedback animated fadeIn">{{ $message }}</div>
        ...
        $('.row-block:last').after(`<div class="row row-block mt-4">
                                        <div class="col-sm-9">
                                            ...
                                            <select name="code[]" id="code.${index}" class="form-control js-select2 @error('code.${index}') is-invalid @enderror">
                                                <option value="" selected disabled>-- Pilih --</option>
                                                @foreach ($bm as $item)
                                                    <option value="{{ $item->id }}" {{ old('code.${index}') == $item->id ? 'selected' : '' }}>{{ $item->code }} - {{ $item->name }} - ({{ $item->qty }})</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-sm-2">
